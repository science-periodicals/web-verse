...
export function getScope(nodeOrRange) {
  var $scope = nodeOrRange.nodeType
    ? nodeOrRange
    : nodeOrRange.commonAncestorContainer;
  // get closest Element
...
export function rangeFromOffsets($scope, startOffset, endOffset) {
  ...
  while ((node = it.nextNode())) {
    ...
    if (
      relStartOffset === undefined &&
      accumulator + tc.length >= startOffset
    ) {
      startNode = node;
...
function textNodeFromNode($container) {
  ...
  let $node;
  let it = $container.ownerDocument.createNodeIterator(
    $container,
    SHOW_TEXT,
    null,
    true
...
export function getOffsets(range, $scope) {
  ...
  while ((node = it.nextNode())) {
    if (node === startTextNode) {
      rawStartOffset = textNodes
        .map(tn => tn.textContent.length)
        .reduce((a, b) => {
          return a + b;
        }, range.startOffset);
    }
    if (node === endTextNode) {
      rawEndOffset = textNodes
        .map(tn => tn.textContent.length)
        .reduce((a, b) => {
          return a + b;
        }, range.endOffset);
  ...
  return {
    startOffset: normalizeOffset(rawStartOffset, txt),
    endOffset: normalizeOffset(rawEndOffset, txt)
  };
}
...
export function getChildOffsets($parent, $child) {
  ...
  let node,
    ...
    rawEndOffset,
    textNodes = [],
    it = $parent.ownerDocument.createNodeIterator(
      $parent,
      SHOW_TEXT,
      null,
      true
  ...
  return {
    startOffset: normalizeOffset(rawStartOffset, $parent.textContent),
...
export function getRangesFromText($scope, text) {
  ...
  return matchIndexes.map(function(match) {
    return rangeFromOffsets(
      $scope,
      normalizeOffset(match.index, tc),
      normalizeOffset(match.index + match.length, tc)
    );
  });
}
...
export function normalizeOffset(rawOffset, rawText) {
  ...
  let delta =
    workText.length -
    workText.replace(RE_TRIM_LEFT, '').replace(RE_SPACES_GLOBAL, ' ').length;
...
export function denormalizeOffset(normOffset, rawText) {
  ...
  let blocks = rawText.split(RE_SPACES_CAPTURE).map(b => {
    return {
      rawLength: b.length,
      normalLength: RE_ONLY_SPACE.test(b) ? 1 : b.length
    };
  });
  ...
  for (var i = 0; i < blocks.length; i++) {
    ...
    if (block.normalLength + normalisingOffset >= normOffset) {
      return (
        rawOffset +
        (block.rawLength - block.normalLength) +
        (normOffset - normalisingOffset)
      );
    } else {
